...
static std::optional<IndexStoreOptions> getOpts(const RequestDict &Req, ResponseReceiver Rec) {
  ...
  if (auto IncludeSystemModules = Req.getOptionalInt64(KeyIncludeSystemModules)) {
  ...
  if (auto DisableImplicitModules = Req.getOptionalInt64(KeyDisableImplicitModules)) {
...
static void handleRequestIndexToStore(
  ...
  handleSemanticRequest(Req, Rec, [Req, Rec, CancellationToken]() {
    ...
    std::optional<IndexStoreOptions> Opts = getOpts(Req, Rec);
    if (!Opts)
      return;
